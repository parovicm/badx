...
#        print(f"Is_sized: {train_dataset_is_sized}, total_train_batch_size * args.max_steps: {total_train_batch_size} * {args.max_steps}")
...
#        print(f"Epochs trained, num_train_epochs: {epochs_trained}, {num_train_epochs}")        
...
#                print(inputs['input_ids'].size())
    ...
    def stats_sign_scores(
        ...
        """
        Run prediction and returns metrics.
        ...
        Args:
            test_dataset (:obj:`Dataset`, `optional`):
                Pass a dataset if you wish to override :obj:`self.test_dataset`. If it is an :obj:`datasets.Dataset`,
                columns not accepted by the ``model.forward()`` method are automatically removed. It must implement the
                :obj:`__len__` method.
            ignore_keys (:obj:`Lst[str]`, `optional`):
                A list of keys in the output of your model (if it is a dictionary) that should be ignored when
        ...
        Returns:
            A dictionary containing the prediction loss and the potential metrics computed from the predictions. The
            dictionary also contains the epoch number which comes from the training state.
        """
        # memory metrics - must set up as early as possible
        # self._memory_tracker.start()
        ...
        for _ in range(num_trials):
            test_dataset = test_dataset_orig.shuffle(seed=random.randint(10, 100000)).select(range(num_examples // 2))
            metrics = self.evaluate(test_dataset)
            if f"{metric_key_prefix}_accuracy" in metrics:
                scores1.append(metrics[f"{metric_key_prefix}_accuracy"])
            if f"{metric_key_prefix}_f1" in metrics:
                scores2.append(metrics[f"{metric_key_prefix}_f1"])
            if f"{metric_key_prefix}_uas" in metrics:
                scores1.append(metrics[f"{metric_key_prefix}_uas"])
            if f"{metric_key_prefix}_las" in metrics:
                scores2.append(metrics[f"{metric_key_prefix}_las"])
