...
class ModelArguments:
    ...
    do_stats_sign: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to do statistical significance (currently only supporting acc/f1/uas/las scores)"},
    )
    stats_sign_file: Optional[str] = field(
        default='file.pkl',
        metadata={"help": "File to output statistical significance (acc/f1/uas/las) scores"},
    )
...
def main():
    ...
    if model_args.do_stats_sign:
        logger.info("*** Statistical significance scores ***")

        metrics = trainer.stats_sign_scores(dataset["test"], num_trials=100)
        if len(metrics[1]) > 0:
            metrics = np.vstack((metrics[0], metrics[1]))
        else:
            metrics = np.array(metrics[0])
        pickle.dump(metrics, open(os.path.join(training_args.output_dir, model_args.stats_sign_file), "wb"))
